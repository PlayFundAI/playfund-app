...
- [x] **Checked whether "split with another payer" (one of the 4 decline-fork options) actually works — it doesn't need a live test, the code already answers it.** `showDeclineSplitInfo()` is a placeholder: clicking it shows an alert saying "Splitting a payment with another person isn't available in the app yet." The other two fork options are real — "Pay the club directly" hits `POST /athlete/:id/notify-club`, "Apply for a scholarship" hits `POST /scholarship/apply` — so split-with-another-payer is the one gap in an otherwise-working fork. Worth deciding whether to build it for the pilot or just make sure the decline screen's copy doesn't oversell it in the meantime
- [ ] **Separate bug found while setting this up:** Supabase's own Auth redirect allowlist still only has the dead `jacksonwatkins30.github.io` URL — even though the Worker now sends the correct `playfundai.github.io` URL as `redirect_to` on every club invite link, Supabase silently substitutes its own configured default instead of erroring. This means every new club's real welcome-email invite link is currently sending admins to a dead page. Fix is in the Supabase dashboard, not code: Authentication → URL Configuration → add `https://playfundai.github.io/playfund-app/**` to Redirect URLs (and update Site URL)
