...
- [ ] **New, more serious finding while verifying that fix: roughly half of real invite emails silently die before anyone clicks them, and it's not the redirect-host bug.** Isolated by testing three separate variables directly, not guessed: (1) a link generated and tested without ever going through email — 20/20 successes; (2) an `invite`-type link vs. `magiclink`-type — no difference, both fine in isolation; (3) toggling Resend's click tracking on/off — no difference either way, ruling out my first theory that click-tracking was pre-fetching/consuming the link. The one variable that actually correlates with failure: whether a real email carrying the link gets delivered to a real Gmail inbox. Across ~9 real `/club/register` trials with a real email send, roughly half of the resulting links came back `otp_expired`/`access_denied` on the very first click. This matches a well-known category of problem with single-use magic-link auth: Gmail (and similar providers, plus corporate mail gateways) commonly pre-fetch/scan links in incoming email for safety, which silently burns a single-use token before the real recipient ever clicks it. **This is not something a settings toggle fixes** — it needs an actual auth-flow change: either a numeric one-time code the person types in (immune to link-prefetching since nothing can "type" a code on their behalf) instead of/alongside the magic link, or Supabase's documented two-step confirm pattern (the link lands on a page requiring an explicit click before the token is actually consumed, rather than being consumed by the GET request itself). Real, current impact: **about half of real club admins clicking their real "Set up your account" email link right now will hit a dead error instead of logging in** — this is a bigger and more urgent problem than the redirect-host bug it was found while double-checking
