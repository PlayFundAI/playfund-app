...
**Known cleanup owed:** several throwaway test clubs now exist in Supabase from testing passes across this session — everything named "Claude QA Test Club ... (delete me)" (numbered up into the 30s at last count, from the batches of registrations used to isolate the magic-link OTP bug). All inactive, harmless, but unused clutter. Filter Supabase's `clubs` table by name containing "delete me" and remove them whenever convenient — not urgent. Also worth a glance: this involved sending ~30 real test emails in rapid succession to `ajjurko4@gmail.com` (via `+alias` addresses) — if Gmail's spam/security systems flagged that pattern, it's worth knowing about before drawing conclusions from any near-term real signup test.
